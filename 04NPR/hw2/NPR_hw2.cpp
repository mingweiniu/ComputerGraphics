			...
			for (int y = 1; y < HEIGHT - 1; ++y) {
				...
				for (int c = 0; c < static_cast<int>(color::size); ++c) {
					...
					for (int i = 0; i < neighbor; ++i) {
						auto speed = vertices[potions[i]].surface[c] - vertices[local].surface[c];
						if (speed > 0.05) {
							vertices[local].surface[c] += speed / 2;
							vertices[potions[i]].surface[c] -= speed / 2;
						}
					}
					// dry
					if (vertices[local].surface[c] > 0.4) {
						vertices[local].surface[c] -= 0.05;
						vertices[local].middle[c] += 0.05;
					}
				}
			}
		}
...
void key_movement()
{
	if (keys[GLFW_KEY_C]) { std::thread T(clean);	T.join(); }
